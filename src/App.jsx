...
const DatabaseChatWindow = ({ messages, sendMessage, setUrl }) => {
  ...
  setUrl(`http://127.0.0.1:8005/api/v1/sql-query/1/${session_id}`);
  return (
...
const App = () => {
  ...
  const [url, setUrl] = useState("http://127.0.0.1:8005/api/v1/query/1/");
  const location = useLocation();
  const navigate = useNavigate();

  const sendMessage = async (userInput) => {
    // const response = await axios.get(`http://127.0.0.1:8005/api/v1/user/1/conversations`);
